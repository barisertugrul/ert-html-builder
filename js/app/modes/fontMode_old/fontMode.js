define(['services/htmlCleanerService', 'modes/fontMode/breakpoint', 'modes/fontMode/cssChanger', 'modes/fontMode/sideBar', 'modes/fontMode/variables', 'modes/fontMode/fontGetter'],
    function (htmlCleanerService, breakpoint, cssChanger, sideBar, variables, fontGetter) {
        ...
        return {
            ...
            fontTagsInit: function () {
				...
				var demo = $('.demo');
                var demoVariables = $(demo).find('h1,h2,h3,h4,h5,h6,p,a')
			...
			resetBehaviour: function() {
				...
				function resetAll() {
					$('#font-media, #font-styles').text('');
					$('#iframePreviewMode').find('iframe').contents().find('head').find('#mediaStyles, #font-styles, #font-face').text('');
					$('.font-settings input[name=fonts]:checked').prop('checked', false);
					sideBar.sideBarRefresh();
				}
