define(['../../../toastr', 'modes/fontMode/sideBar', 'services/htmlCleanerService'], 
    ...
    var currentMedia,
        frame,
        fontTags = '<div class="demo"><div class="font-tags"><div class="font-body-holder"><div class="font-body"><h1>Heading One H1</h1><h2>Heading Two H2</h2><h3>Heading Three H3</h3><h4>Heading Four H4</h4><h5>Heading Five H5</h5><p>Paragraph. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Duis mollis, est non commodo luctus.Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor. Duis mollis, est non commodo luctus.</p><a class="margin-top-10 btn btn-info">BUTTON</a></div></div></div></div>';
    var self = {
        init: function () {
            self.frameSettings();
		...
		setValues2frame: function () {
			...
			head.find('#font-styles').append( fs[0] );
        ...
        addButtonsBehavior: function () {
           ...
           var devicesElements = $('.font-settings').find('a.media-w-btns');
            devicesElements.on('click', function (e) {
                ...
                self.log.info('All changes made now will be available for viewing' + infoScreen + ' media', 'Info')
        ...
        addStyles: function () {
            $(frame).contents().find('head').find('#font-styles').html($('#font-styles').text().replace(/(.demo \w*?,.font-body)/g, 'html body'));
        },
