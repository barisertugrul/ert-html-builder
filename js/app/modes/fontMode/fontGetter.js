        ...
        /*getFonts: function () {
		 ...
		 getFonts: function () {
		 	...
		 	var fontsArray = [
				...
				var cList = $('ul.mylistFonts')
				var y=0;

				for (var i = 0; i < fontsArray.length; i++) {
					var classFont = fontsArray[i].replace(" ", "_");

                cList.append('<li class="'+ classFont +'"  style="background-position:-10px -'+y+'px !important;"  title="' + classFont + '">' + classFont + '</li>');
				y +=29;

            }

			var fontLiItem = $(cList).find('li');
			fontLiItem.off('click').on('click', function(){
				$(cList).find('li.active').removeClass('active')
				itemId = $(this).attr('class');
				$(this).addClass('active');
				var fontLoaderItem = $('.font-control').find('#' + itemId + '');
				console.log(fontLoaderItem)
				$(fontLoaderItem).trigger('click')
