    ...
    var self = {
        ...
        changeCommonCss: function (attr, val) {
            ...
            if (styles.indexOf(attr) < 0) {
                styles = styles.replace(/}/, attr + ':' + val + ' !important;}');
            } else {
                var regAttr = new RegExp(attr + '\\s?:\\s?.*?;');
                styles = styles.replace(regAttr, attr + ':' + val + ' !important;')
            }
					...
					if (allMediaArr[i].indexOf(pre) > -1) {
						if (allMediaArr[i].indexOf(tagSelect) > -1) {
							...
							if (style.indexOf(attr) > -1) {
								style = style.replace(regAttr, attrVal + ' !important;');
							} else {
								style = style.replace('{', '{' + attrVal + ' !important;');
							}
							allMediaArr[i] = allMediaArr[i].replace(regStyle, style);
						} else {
							allMediaArr[i] += '}' + tagSelect + '{' + attrVal + ' !important;';
						}
					}
				}
				fontMedia.html( allMediaArr.join('}}') );
			} else {
				fontMedia.append('\n' + pre + '{' + tagSelect + '{' + attrVal + ' !important;}}');
			}
