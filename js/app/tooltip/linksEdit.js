define(['../services/videoEmbedService'], function (videoEmbedService) {
    return {
        show: function (element) {
            var oldLink = $(element).attr('href');
            var dataVideo = $(element).attr('data-lightbox-content');
            $('#link-data').val(oldLink);
			if(dataVideo){
				$('#link-video').val(dataVideo);
			}
			
            else{$('#link-video').val('');}
			//var newVideoData = $('#link-video').val();
            $('#link-change-modal').modal('show');
			 $('#save-link').off('click').on('click', function () {
				...
				/*if (newSrc != '') {
					...
					*/
					//var urlYT = 'https://www.youtube.com/embed/' + $('#link-video').val()
					//$(element).attr('data-lightbox-content', urlYT);
					var urlYT = $('#link-video').val();
					 var finalSrc = videoEmbedService.convertMedia(urlYT);
						
						$(element).attr('data-lightbox-content', finalSrc);
						if($(element).hasClass('lightbox-toggle')){}
						else{$(element).addClass('lightbox-toggle').removeClass('scroll')}
				
				}
				else{
					$(element).removeAttr('data-lightbox-content');
					$(element).removeClass('lightbox-toggle')
				}
							...
							event.preventDefault();
										...
										if ($(thisElement).attr('id')) { 	
											...
											$('#log-anchor').html('<i class="mdi-image-filter-2"></i> This Section ID <trong>#' + text + '</strong>  Click <strong>SAVE</strong> on the form to use this ID as link anchor');
												...
												var newLinkVal = $('#inputLinkAnchor').prop('value');
												...
												$('#inputLinkAnchorBtn').off('click').on('click', function(){
													...
													$(thisElement).attr('id', newLinkVal);
													
													$(element).attr('href', '#' + newLinkVal);
